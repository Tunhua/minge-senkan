...
t+=1;
		if(Input.GetKey (KeyCode.A)&&t>5){
			...
			vecC =  GameObject.Find("muzzle3").transform.position;
			vecM =  GameObject.Find("target3").transform.position;
...
// Rigidbodyに力を加えて発射
	...
	// 弾丸の位置を調整
		lazers.transform.position = GameObject.Find("target3").transform.position;
