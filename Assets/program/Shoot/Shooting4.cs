...
// bullet prefab
	...
	void Update () {
		...
		if(Input.GetKey (KeyCode.A)&&t>5){
			...
			vecC =  GameObject.Find("muzzle4").transform.position;
			vecM =  GameObject.Find("target4").transform.position;
...
// Rigidbodyに力を加えて発射
	...
	// 弾丸の位置を調整
		lazers.transform.position = GameObject.Find("target4").transform.position;
